...
def build_deck_csv(
    ...
    for (county_name, region_name) in county_regions:
        try:
            ...
            raise Exception(f"Key error, could not find region {region_name} in region_country for county {county_name}")
    ...
    for (bow_name,) in body_of_water_names:
        rows.append(
            {"location": bow_name, "bow": bow_name, "tags": "BoW"}
        )
    ...
    assert set([c for (c,) in city_names]) == set(counties_by_city.keys())
